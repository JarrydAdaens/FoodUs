# 2026-07-27 - FoodUs Relay Tier-0 Seed (server repo split addendum)

## Source

- Captured from: External tier-0 design document authored during the 2026-07-27 relay
  infrastructure discussion session, delivered as
  `E:\Downloads\2026-07-27_foodus-relay_tier-0-design.md`. The document is the seed for the
  **foodus-relay** repository — a new, separate server repo with its own agentic-rails setup —
  and is preserved here because it also imposes obligations on this app repo.
- Related project area: Milestone 3 (Multiplayer), Stories 4-15; design.md "The Multiplayer
  Exception".

## Raw Notes

The source document is preserved verbatim below. Its original YAML front matter is shown as a
fenced block so this intake file keeps a single front-matter-free header.
...
- New decision: relay stack and hosting resolved — ASP.NET (C#) minimal API + SQLite behind
  Caddy TLS on a DigitalOcean Sydney droplet; formerly open in Milestone 3 Story 4.
- New decision: the relay lives in its own repository (`foodus-relay`) with its own
  agentic-rails structure, and that repo **owns the wire contract**; this app repo conforms.
- New decision: two rooted agents, one per repo; edits happen at home; the owner is the human
  relay for contract changes and enforces "server leads, app follows" deployment ordering.
- New decision: paired cross-repo stories share a common slug, and app child stories carry a
  one-way versioned dependency note against their server parent story.
- Changed assumption: app-repo Story 4's remaining open decisions and Story 5's delivery move
  into the foodus-relay repo; in this repo they become external dependency gates, not
  executable app work.
- Possible milestone or story impact: seven explicit app obligations (configurable relay URL
  setting; contract conformance; envelope version refuse-loudly; two-way field tolerance;
  capability-aware UI; HTTPS only; dependency notes) land in Milestone 3.
- Open question: unknown-envelope-version disposition — when the app refuses an envelope
  version loudly, is the message acknowledged off the mailbox (data loss) or left queued
  (risking a poll-error loop)? A contract decision owned by foodus-relay; flagged during
  synthesis 2026-07-27, not covered by the source document.

## Integration Notes

- Update `../design.md` (and its Milestones Index): Multiplayer Exception gains the
  contract-ownership pointer to foodus-relay, the resolved stack/hosting decision, the
  user-entered private relay URL, versioning/tolerance/capability rules, the deployment
  ordering rule, and the two-agent organisation; External services and Configuration entries
  updated to match. Done 2026-07-27.
- Update `../milestones/`: milestone-3.md — Stories 4 and 5 converted to external dependency
  gates on foodus-relay; a Relay Contract Conformance section added; dependency notes added to
  relay-consuming stories; new Story 15 (configurable relay URL setting); interdependency
  order updated. Done 2026-07-27.
- Update `../backlog/`: no changes — deferred items (push transport, N-person groups, safety
  numbers, bios) were already staged in backlog-1 by the 2026-07-27 multiplayer addendum.
- Update `../implementation-plans/`: no changes — Milestone 3 plans are written at execution
  time and must cite this dictation plus the relay repo's contract spec.
