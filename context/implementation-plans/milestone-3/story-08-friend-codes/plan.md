...
- Story: [Story 8: Friend codes](../../../milestones/milestone-3.md#story-8) `[STORY 3.8]`
...
- Related Plans:
  - Story 3.2 (Profile) — `../story-02-profile/plan.md` (planned in the same run): owns the profile record this story's friend code lives on and the My Profile card that displays it.
  - Story 3.7 (Relay URL setting) — `../story-07-relay-url-setting/plan.md` (planned in the same run): owns the user-entered relay endpoint and the capability check that gates this story's server calls.
  - Story 3.9 (Friends list) — consumes friend codes for add-by-code resolve; not planned here.
- External Tooling: none required.

**Dependency note (milestone-mandated, app → server):** blocked by foodus-relay: friend-code endpoints (shared slug `...-friend-codes`), contract v1, deployed. The owner releases this block via the Story 4 gate. Also requires Story 3.7's relay URL configured on-device. Contract v1 is **Not Started** in the foodus-relay repo as of 28 July 2026 — every server-facing detail below is a contract-dependent seam, not a specification.
...
- Capability gating: friend-code display/regenerate degrade gracefully when the relay is unset or unreachable (Story 3.7's capability check) — regenerate hidden or greyed per the capability-aware UI rule.
...
- Resolving a *friend's* code into GUID/username/public key — Story 3.9 (add-by-code).
- The profile record, My Profile card construction, and backup inclusion — Story 3.2.
- The relay URL setting, HTTPS validation, and the version/capability probe — Story 3.7.
- Envelope schema, version stamping, mailbox transport — Story 3.10 (blocked; unplanned this run).
...
- **Ktor convention:** per-consumer `HttpClient` registered under a named qualifier in the slice's Koin module — `USDAModule.kt:14` and `AiInfrastructureModule.kt:24` both do `single(named(...)) { HttpClient { install(HttpTimeout); install(ContentNegotiation) { json(Json { ignoreUnknownKeys = true }) } } }`. The relay client follows this pattern; HTTPS-only is enforced by Story 3.7's URL validation upstream of any call.
...
- Q: `[STORY 3.8]` Friend-code minting authority — server-assigned, or client-generated + registered? (Contract-owned: transferred to foodus-relay's first milestone; owner input wanted there.)
...
- Q: `[STORY 3.8]` Friend-code alphabet and case rules (shape fixed at 4-4-4 with dashes; exact charset pending). (Contract-owned.)
  Impact: Input-normalization for display and, in Story 3.9, for code entry (e.g. is `a7f3` the same as `A7F3`? Are ambiguous glyphs like `0/O`, `1/I` excluded?). Blocks any local generation or strict validation logic.
...
- Q: `[STORY 3.8]` When is a code first provisioned — at profile creation (requires relay configured, contradicting Story 3.2's local-only creation) or lazily, the first time the relay is configured and reachable?
  ...
  Assumption: Lazy provisioning — profile creation stays local-only (Story 3.2); the code slot shows a "connect a relay to get your friend code" placeholder until the Story 4 gate is released and the first registration succeeds.
  Status: OPEN
  Answer: —

- Q: `[STORY 3.8]` Should the display include a copy-to-clipboard affordance? The story mandates display + regenerate only, but a 12-character code is painful to transcribe by eye.
...
> Steps 1–4 are buildable now (local + seam). Step 5 is contract-gated and must not start before contract v1 is carried across and the Story 4 gate is released ("server leads, app follows").

1. Domain model: friend-code value type + shape validation in the social slice domain.
   - Why: One canonical representation for storage, display, and (later) Story 3.9 input parsing.
...
4. UI: friend-code display + regenerate flow on the My Profile surface.
   ...
   - Edits: Friend-code row (formatted display, copy affordance per open question, placeholder when unprovisioned) added to Story 3.2's profile screen; regenerate action behind an are-you-sure dialog ("old copies stop working; your friends are unaffected"); action hidden/greyed when the relay capability check fails (Story 3.7). New additive strings in `strings.xml`.
...
5. **[CONTRACT-GATED]** Infrastructure implementation of `FriendCodeRelayClient` per contract v1.
   ...
   - Dependencies: foodus-relay contract v1 carried across by the owner; Story 4 gate released; Story 3.7's relay URL + capability check live.
...
- Unit test: regenerate flow persists the new code and never mutates friend records (once Story 3.9's friend storage exists, guard the invariant "regeneration touches only the profile's own code").
...
2. (Post-gate) With a live relay: code appears after provisioning; regenerate produces a new code; the old code no longer resolves (verified from the second phone via Story 3.9's add-by-code).
3. (Post-gate) Regenerating with an existing friendship intact: the friendship survives (Story 3.15 household proof covers this end-to-end).
...
- Planning inputs: milestone-3.md Story 8 + Relay Contract Conformance; design.md "The Multiplayer Exception".
