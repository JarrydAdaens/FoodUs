...
- Story: [Story 10: Save to Group](../../../milestones/milestone-3.md#story-10) `[STORY 3.10]`
- Design authority: `context/design.md` — "The Multiplayer Exception" (relay model, E2E, poll-on-wake, Relay Contract Conformance)
- Related Plans:
  - Story 9 (Groups) — **depends on it**: supplies the group Room storage, trust levels, and membership this screen lists. Planned in parallel at `context/implementation-plans/milestone-3/story-9-groups/`.
  - Story 8 (Message envelope & E2E pipeline) — **depends on it**: supplies the send path. Its plan is BLOCKED this run (wire contract v1 is Not Started in foodus-relay); this plan consumes it strictly as an interface dependency and invents no envelope or wire details.
  - Story 15 (Configurable relay URL setting) — capability gating source: the Save to Group surface hides or greys when the relay is unset/unreachable per the capability-aware UI rule.
- Dependency note (Relay Contract Conformance rule 6): consumes the relay **only through Story 8's pipeline** (contract v1); no new server capability of its own. Dependency arrow app → server; owner releases via the Story 5 gate.
...
- Group-selection card UI above the save controls on the add-entry screen: user's groups with checkboxes, collapsed/absent when the user has no groups or the relay is unset (capability gating per Story 15).
...
- A domain-level send port (e.g. a `SendEntryToGroup`-shaped use case interface in the fork's multiplayer slice) that carries: the complete entry payload source (`DiaryFood` nutrition + name), entry timestamp, meal name, and target group/member identity. **Interface only** — its implementation, envelope schema, queuing, and encryption are Story 8's.
- New English strings (fork-owned keys) in the shared resources base `strings.xml`.

### Out Of Scope

- The packet schema, encryption, send queue, retry/offline semantics, and any relay endpoint interaction — all Story 8 (blocked on contract v1).
- Group data model, trust levels, membership storage — Story 9.
- Recipient-side delivery behavior (Full auto-insert, Suggest queue) — Stories 11 and 12.
- The relay URL setting and capability check — Story 15.
...
- No send-status tracking UI beyond what Story 13's Notification Center later surfaces.
...
- **Groups source:** Story 9 plan (parallel) owns group + membership Room storage and trust levels; this plan consumes an observe-groups query for the checkbox card.
...
- Q: `[STORY 3.10]` Do None-trust groups appear in the Save-to-Group checkbox list — hidden entirely, or shown greyed out? *(Staged open decision from the milestone; owner input wanted.)*
...
- Q: `[STORY 3.10]` Does Save to Group also save the entry into the sender's own diary, or only transmit?
...
- Q: `[STORY 3.10]` Which entry-creation surfaces get Save to Group? `AddEntryScreen` only, or also quick-add/manual entries (`quickadd/`), fast text, and entry edits (`update/`)?
...
- Q: `[STORY 3.10]` The dictated layout ("big Save to Group button on the left, Save on the right, card above") conflicts with the screen's current single-FAB-column idiom. Literal two-button bottom bar, or FAB-idiom adaptation (e.g. secondary FAB / bottom action row)?
...
- Q: `[STORY 3.10]` Behavior when the relay is unreachable at send time (send queue? fail loudly?).
  Impact: User-visible reliability of the marquee feature.
  Assumption: Owned by Story 8's pipeline (send queue semantics); this UI only reports what the port returns and never blocks local save. Dependency, not invention.
  Status: OPEN
  Answer: —
...
1. Define the consumer-side send port in the fork's multiplayer domain (new file, fork-owned).
   - Why: Lets this story compile and be UI-complete while Story 8 is blocked; the port is the seam the pipeline later implements.
   - Edits: new interface (e.g. `SendDiaryEntryToGroupMember`) taking recipient identity, group id, `DiaryFood` snapshot, entry timestamp, meal name; a no-op/queued stub binding in the fork's Koin module until Story 8 lands.
   - Dependencies: naming/ownership aligned with Story 9's group model; revisit signature when Story 8 is planned.

2. Add an observe-user-groups query consumption for the checkbox card.
   - Why: The card lists the user's groups with checkboxes.
   - Edits: inject Story 9's group repository/use case into `AddEntryViewModel`; expose `StateFlow` of selectable groups (filtered per the None-trust decision) + checkbox state.
   - Dependencies: Story 9 storage exists.
...
3. Build the group-selection card composable (new fork-owned file under `app/ui/food/diary/add/`).
   ...
   - Edits: card with group checkboxes, rendered only when groups exist and relay capability allows (Story 15 gating); persisted checkbox state across recomposition (`rememberSaveable`/ViewModel).
...
3. Save to Group with two checked groups triggers local save once and two port invocations (stub-logged until Story 8 exists).
4. Relay unset (Story 15): Save to Group surface hidden/greyed.
...
- No envelope, endpoint, or crypto detail exists in this story's code — everything relay-facing goes through the Story 8 port.

## Risk Mitigation

- Risk: Story 8's real pipeline interface diverges from the port defined here.
  Mitigation: Keep the port minimal (recipient, group, snapshot, timestamp, meal name — exactly the story-mandated packet content); adjust in the same sitting Story 8 is planned, before any implementation.
